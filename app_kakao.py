...
3. 하단 **채팅방 설정** 을 클릭합니다.
4. **대화 내용 관리 > 대화 내용 저장 > 텍스트 파일로 저장**을 클릭합니다.
5. 저장된 텍스트 파일(.csv)를 아래에 업로드하세요.
6. 새로운 채팅방을 분석할 때에는 이전에 업로드한 채팅방 파일을 삭제해야합니다.

## 📋 업로드된 파일은 제작자 혹은 제 3자에게 공유되거나 저장되지 않습니다.
Gemini 내부에서 처리된 후 파기됩니다. 의심스럽다면 코드 직접 확인해보셔도 좋습니다.
[제작자 깃허브 바로가기](https://github.com/ODDITY0815/kakao_wrapped.git)

### 🎯 주요 기능

- **🎁 Wrapped**: 연도별 대화 통계, MVP, 최다 단어, 황금 시간대 등을 카드 형식으로 확인
- **🎭 성격 분석**: AI가 각 멤버의 대화 패턴을 분석하여 MBTI, 특수능력, 캐릭터 프로필 생성
- **🤖 심층 리포트**: 대화방의 전반적인 분위기, 주요 관심사, 한 줄 총평을 AI가 작성
- **💬 챗봇**: 대화 내용을 기반으로 "누가 언제 뭐라고 했는지" 자유롭게 질문하고 답변 받기
- **📊 발화량**: 사용자별 메시지 개수를 막대 그래프로 시각화
- **☁️ 키워드**: 대화에서 가장 많이 언급된 단어 Top 30을 그래프와 표로 확인
- **📋 데이터**: 업로드한 원본 대화 데이터를 테이블 형태로 조회
